...
const FIELD_TYPES_SMART = [
  ...
  { value: "stat_add", label: "📊 Addition (A + B)" },
];
...
export function AuditFormBuilder({ auditTypeKey }: Props) {
  ...
  const isStatAdd = fieldType === "stat_add";
  const needsSources = isStatPercent || isStatSum || isStatDiff || isStatAdd;
                  ...
                  <div className="space-y-3">
                    {isStatPercent ? (
                      ...
                      <>
                        <Label>Champ A</Label>
                        <Select value={sourceNumerator} onValueChange={setSourceNumerator}>
                          <SelectTrigger><SelectValue placeholder="Champ A" /></SelectTrigger>
                          <SelectContent>
                            {numberFields.map((nf) => (
                              <SelectItem key={nf.id} value={nf.id}>{nf.field_label}</SelectItem>
                            ))}
                        ...
                        <Label>Champ B</Label>
                        <Select value={sourceDenominator} onValueChange={setSourceDenominator}>
                          <SelectTrigger><SelectValue placeholder="Champ B" /></SelectTrigger>
                          <SelectContent>
                            {numberFields.map((nf) => (
                              <SelectItem key={nf.id} value={nf.id}>{nf.field_label}</SelectItem>
                            ))}
                      ...
                      <>
                        ...
                        <p className="text-xs text-muted-foreground">Résultat = A + B.</p>
                      </>
                    ) : (
                      <>

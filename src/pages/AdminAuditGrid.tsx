...
import {
  faPlus, faTrashCan, faPenToSquare, faFloppyDisk, faChevronLeft, faGripVertical, faLayerGroup,
...
export default function AdminAuditGrid() {
  ...
  // Load types
  useEffect(() => {
    supabase.from("audit_types").select("*").order("key").then(({ data }) => {
      setTypes(data || []);
  ...
  if (loading) {
    return <AppLayout><div className="flex items-center justify-center py-20"><p className="text-muted-foreground animate-pulse">Chargement…</p></div></AppLayout>;
  }
    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto py-8 px-4 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold">Grilles d'audit</h1>
            <p className="text-xs text-muted-foreground">Gérez les catégories et items par type d'événement</p>
          </div>
        </div>
